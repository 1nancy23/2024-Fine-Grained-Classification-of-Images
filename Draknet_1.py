...
class Get_UpDwconv(nn.Module):
    def __init__(self):
        super(Get_UpDwconv, self).__init__()

        self.classifier1 = nn.Sequential(
            GlobalAvgPool2d(16, 1024),
            nn.ReLU(True),
            nn.Linear(1024, 256)
        )
        self.classifier2 = nn.Sequential(
            GlobalAvgPool2d(32, 512),
            nn.ReLU(True),
            nn.Linear(512, 256),
        )
        ...
        self.tconv3_1=nn.ConvTranspose2d(in_channels=1024, out_channels=512, stride=2, kernel_size=5, padding=2,
                                          output_padding=1, dilation=1, padding_mode="zeros", bias=False)
        self.tconv2_1=nn.ConvTranspose2d(in_channels=512, out_channels=256, stride=2, kernel_size=5, padding=2,
                                          output_padding=1, dilation=1, padding_mode="zeros", bias=False)
        self.conv3_1=nn.Conv2d(in_channels=1024,out_channels=1024,kernel_size=3,padding=1)
        self.conv2_1=nn.Conv2d(in_channels=512,out_channels=512,kernel_size=5,padding=2)
        self.conv1_1=nn.Conv2d(in_channels=256,out_channels=256,kernel_size=7,padding=3)
        self.conv1_2= nn.ConvTranspose2d(in_channels=256, out_channels=256, stride=2, kernel_size=5, padding=2,
                                          output_padding=1, dilation=1, padding_mode="zeros", bias=False)
        self.conv1_3=nn.ConvTranspose2d(in_channels=256, out_channels=256, stride=2, kernel_size=5, padding=2,
                                          output_padding=1, dilation=1, padding_mode="zeros", bias=False)
        self.Att = ScaledDotProductAttention(256)
    def forward(self,features):
        # for i in features:
        #     print(i.shape)
        #features=[x1=1024,x2=512,x3=256,y1=1024,y2=512,y3=256]
        x3=self.conv3_1(features[0]+features[3])
        x2=self.conv2_1(self.tconv3_1(x3)+features[1]+features[4])
        # print('x2',x2.shape)
        Up_Map=self.Att(self.classifier1(features[0]),self.classifier2(features[1]),self.classifier3(features[2]))
        x1=torch.einsum('ij,ijkl->ijkl', Up_Map,
                     (features[2]))
        x1=self.conv1_1(self.tconv2_1(x2)+features[5]+x1)
        output=self.conv1_3(self.conv1_2(x1))
        return output
class Necks(nn.Module):
    def __init__(self):
        ...
        self.t_conv1 = nn.ConvTranspose2d(in_channels=1024, out_channels=1024, stride=2, kernel_size=5, padding=2,
                                          output_padding=1, dilation=1, padding_mode="zeros", bias=False)
        self.t_conv2 = nn.ConvTranspose2d(in_channels=512, out_channels=512, stride=2, kernel_size=5, padding=2,
                                          output_padding=1, dilation=1, padding_mode="zeros", bias=False)
        ...
        self.channelatten1 = ChannelAttention(1024)
        self.spitialatten1 = SpatialAttention()
        self.channelatten2 = ChannelAttention(512)
        self.spitialatten2 = SpatialAttention()
        self.channelatten3 = ChannelAttention(256)
        self.spitialatten3 = SpatialAttention()
        self.channelatten4 = ChannelAttention(512)
...
class CSPDarkNet53(nn.Module):
    ...
    def forward(self, x):
        ...
        Features=[x3,x2,x1]
        Features.extend(self.Neckm([x1,x2,x3]))###channels=64,128,256
        output=self.head(Features)
        # print(Features[0].shape,Features[1].shape,Features[2].shape)

        return ( output> 0.3).float()
...
# Model1=cspdarknet53(num_classes=256).cuda()
